...
Cara Kerja:
  1. Scan judul artikel yang statusnya 'processed' di 7 hari terakhir.
  ...
  5. Validasi Google News (hitung <item> di RSS).
    ...
    for name, count in qualified.items():
        ...
        try:
            sb.table("entity_candidates").upsert({
                ...
                "mention_count": count,
                "confidence_score": min(0.5 + (count * 0.1), 0.8),
                "status": "pending"
...
def run_gnews_validation(sb: Client):
    ...
    res = sb.table("entity_candidates").select("id, detected_name").eq("status", "pending").limit(50).execute()
    ...
    with httpx.Client(headers={"User-Agent": "Mozilla/5.0"}) as client:
        for c in res.data or []:
            ...
            try:
                ...
                sb.table("entity_candidates").update({"gnews_hit_count": hits}).eq("id", c["id"]).execute()
            except Exception:
                pass
            time.sleep(1.5)

def run_auto_promote_and_config(sb: Client):
    """Promote tokoh + OTOMATIS buat RSS config-nya."""
    print("\n[PROMOTE] Memromosi kandidat dan membuat RSS Config...")
    
    # Ambil kandidat yang memenuhi syarat
    ...
    for c in res.data or []:
        ...
        # 1. Insert ke political_entities
        pe_res = sb.table("political_entities").insert({
            "canonical_name": name,
            "entity_type": "other",
            "is_active": True,
        ...
        # 2. AUTO-CREATE Scraping Config (Menutup cacat yatim piatu)
        ...
        sb.table("scraping_configs").insert({
            "entity_id": entity_id,
        ...
        sb.table("entity_candidates").update({"status": "approved", "promoted_entity_id": entity_id}).eq("id", c["id"]).execute()
...
def main():
    ...
    run_auto_promote_and_config(sb)
