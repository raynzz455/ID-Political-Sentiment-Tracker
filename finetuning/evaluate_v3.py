...
evaluate_v3.py
==============
v3 Evaluation script — compatible with finetune_v3.py output.

Reads:
  - runs/sentiment_v3/lora/        (LoRA adapter)
  - runs/sentiment_v3/tokenizer/    (tokenizer)
  - runs/sentiment_v3/metrics.json  (temperature T)

Evaluates on:
  - finetuning/datasets/dataset_v9.jsonl (1378 rows, 94.4% verified)

Reports:
  - Full-coverage accuracy + macro-F1 + confusion matrix
  - Confidence threshold sweep (kept-accuracy vs coverage)
  - Target: >=97% kept-accuracy at >=80% coverage

Usage:
  python finetuning/evaluate_v3.py --task sentiment --run-dir runs/sentiment_v3
  python finetuning/evaluate_v3.py --task relevancy --run-dir runs/relevancy_v3
"""
import sys
import json
import argparse
...
# Setup paths (same logic as finetune_v3.py)
_SCRIPT_DIR = Path(__file__).resolve().parent
sys.path.insert(0, str(_SCRIPT_DIR / "configs"))
sys.path.insert(0, str(_SCRIPT_DIR))

# Import hyperparams_v3
...
def stratified_split(rows, seed=H.SEED):
    ...
    for lab, items in by.items():
        ...
        ntr = max(1, n - nt - nv)
        tr.extend(items[:ntr])
        va.extend(items[ntr:ntr + nv])
        te.extend(items[ntr + nv:])
    rng.shuffle(tr)
    rng.shuffle(va)
    rng.shuffle(te)
    return tr, va, te
...
def score_all_calibrated(model, tok, rows, labels, T, device=None, max_len=H.MAX_SEQ_LENGTH):
    ...
    for r in rows:
        ...
        enc = tok(premise, hypothesis, truncation=True,
                  max_length=max_len, return_tensors="pt").to(device)
        out = model(**enc)
        p = F.softmax(out.logits / max(T, 0.05), dim=-1)[0].cpu().numpy()
        probs.append(p)
...
def main(task, run_dir):
    ...
    T = metrics.get("temperature", H.TEMPERATURE)
    print(f"Task: {task} | run_dir: {run_dir} | temperature: T={T}")

    # Load base + LoRA
    base = cfg["base_model"]
    print(f"Loading tokenizer from {run_dir / 'tokenizer'}...")
    tok = AutoTokenizer.from_pretrained(str(run_dir / "tokenizer"))
    print(f"Loading base model: {base}")
    model = AutoModelForSequenceClassification.from_pretrained(base)
    print(f"Loading LoRA adapter from {run_dir / 'lora'}...")
    model = PeftModel.from_pretrained(model, str(run_dir / "lora"))
    model = model.merge_and_unload()  # merge LoRA for faster inference

    # Load dataset
    data_path = _SCRIPT_DIR / "datasets" / cfg["data_file"]
    ...
    # Reproduce stratified test split
    ...
    # Find best tau for >=97% kept-accuracy
    ...
    if best:
        ...
        print("   Options: (a) label more gold data, (b) raise tau, (c) add LLM second-pass for deferred.")
    ...
    with open(run_dir / "evaluation_v3.json", "w") as f:
        json.dump(out, f, indent=2)
    print(f"\nSaved -> {run_dir / 'evaluation_v3.json'}")


if __name__ == "__main__":
    ap = argparse.ArgumentParser()
    ap.add_argument("--task", choices=["relevancy", "sentiment"], required=True)
    ap.add_argument("--run-dir", required=True, help="e.g., runs/sentiment_v3")
    a = ap.parse_args()
